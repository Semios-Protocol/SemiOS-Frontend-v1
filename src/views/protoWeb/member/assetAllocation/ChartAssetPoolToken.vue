<template>
  <div class="charts-box">
    <div class="chat-title">
      <h3>{{ $t('nodeDetail.subNodesAssetPoolERC20TokensLabel') }}</h3>
    </div>
    ...
    <div v-else class="chart-body">
      <div v-show="chartData && chartData.xdata && chartData.xdata.length > 0">
        <div id="ChartAssetPoolToken" :style="{ height: Mheight }" />
      </div>
      <chart-no-data
...
import { analyticsAssetPoolToken } from '@/common/api'
import dayjs from 'dayjs'
export default {
  name: 'ChartAssetPoolToken',
  components: {
    ChartLoading,
    ChartNoData,
  },
  ...
  methods: {
    reResizeFn() {
      let _ref = document.getElementById('ChartAssetPoolToken')
    ...
    async getData() {
      this.isLoading = true
      const res = await analyticsAssetPoolToken({
      ...
      this.option = {
        ...
        legend: [
          ...
          },
          {
            ...
            data: ['Asset Pool ERC20 Token Balance'],
          },
          {
            left: '20px',
            top: '46px',
        ...
        series: [
          {
            name: 'Asset Pool ERC20 Token Balance',
            type: 'line',
            symbol: 'circle',
            symbolSize: 1,
            // yAxisIndex: 1,
