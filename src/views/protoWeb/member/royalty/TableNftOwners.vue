      ...
      <div v-if="tableData && tableData.length > 0" style="height: 100%">
        ...
        >
          ...
          <el-table-column align="right" width="120">
            ...
            <template slot-scope="scope">
              <span style="color: #9e9e9e">
                ...
                {{ scope.row.userAddress | ellipsis }}
              </span>
            </template>
          </el-table-column>
          <el-table-column align="right">
            ...
            <template slot-scope="scope">
              <span style="color: #9e9e9e"
                >{{ scope.row.erc20Balance | bigNumFormat(2, 0.01) }}
              </span>
            </template>
          </el-table-column>
          <el-table-column align="right">
...
import { erc20Owners } from "@/common/dexApi";
  ...
  methods: {
    ...
    nameFilter(item) {
      if (item.userName) {
        ...
        const name = item.userAddress.slice(2, 8).toUpperCase();
        return name;
      }
    },
    aPush(item) {
      return `${OPEN_URL}/address/${item.userAddress}`;
    },
    goDao(item) {
      const path = "nfts";
      this.$router.push({
    ...
    tableHeaderCellStyle() {
      return { backgroundColor: "#1B2233" };
    },
    async getData() {
      this.tableData = [];
      this.isLoading = true;
      const query = { erc20Address: this.address, dayTime: this.dayTime };
      const data = await erc20Owners(query);
