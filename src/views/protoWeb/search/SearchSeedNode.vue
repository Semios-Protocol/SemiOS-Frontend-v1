<template>
  <div v-if="amountObj.seedNodesAmount > 0" class="nei-box">
    <SeedNodeCard :list="list" :isAll="isAll" :isLoading="loading" />
  </div>
...
import { searchSeedNodes } from '@/common/api'
export default {
  ...
  methods: {
    ...
    async getData() {
      this.list = []
      this.loading = true
      const data = await searchSeedNodes(this.$route.query.query)
