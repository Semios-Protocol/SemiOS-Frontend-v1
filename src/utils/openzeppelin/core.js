...
exports.renderMerkleTree =
  exports.isValidMerkleTree =
  exports.processMultiProof =
  exports.getMultiProof =
  exports.processProof =
  exports.getProof =
  exports.makeMerkleTree =
    void 0;
...
const hashPair = (a, b) =>
  (0, keccak_1.keccak256)(
    (0, utils_1.concatBytes)(...[a, b].sort(bytes_1.compareBytes))
  );
...
const parentIndex = (i) =>
  i > 0
    ? Math.floor((i - 1) / 2)
    : (0, throw_error_1.throwError)("Root has no parent");
const siblingIndex = (i) =>
  i > 0
    ? i - (-1) ** (i % 2)
    : (0, throw_error_1.throwError)("Root has no siblings");
...
const isValidMerkleNode = (node) =>
  node instanceof Uint8Array && node.length === 32;

const checkLeafNode = (tree, i) =>
  void (
    isLeafNode(tree, i) || (0, throw_error_1.throwError)("Index is not a leaf")
  );
const checkValidMerkleNode = (node) =>
  void (
    isValidMerkleNode(node) ||
    (0, throw_error_1.throwError)(
      "Merkle tree nodes must be Uint8Array of length 32"
    )
  );
...
function processMultiProof(multiproof) {
  ...
  if (
    multiproof.leaves.length + multiproof.proof.length !==
    multiproof.proofFlags.length + 1
  ) {
