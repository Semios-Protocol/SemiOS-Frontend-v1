...
function standardLeafHash(value, types) {
  return (0, keccak_1.keccak256)(
    (0, keccak_1.keccak256)(
      (0, utils_1.hexToBytes)(abi_1.defaultAbiCoder.encode(types, value))
    )
  );
}
class StandardMerkleTree {
  ...
  static load(data) {
    ...
    return new StandardMerkleTree(
      data.tree.map(utils_1.hexToBytes),
      data.values,
      data.leafEncoding
    );
  }
  ...
  *entries() {
  ...
  leafLookup(leaf) {
    return (
      this.hashLookup[this.leafHash(leaf)] ||
      (0, throw_error_1.throwError)("Leaf is not in tree")
    );
  }
  getProof(leaf) {
    // input validity
  ...
  getMultiProof(leaves) {
    ...
    return {
      leaves: proof.leaves.map(
        (hash) => this.values[this.hashLookup[(0, bytes_1.hex)(hash)]].value
      ),
